...
type PostEditRouteProp = RouteProp<PostEditParamList, 'PostEdit'>;
...
export default function PostEditScreen() {
  ...
  useEffect(() => {
    const fetchPostData = async () => {
      try {
        setIsLoading(true);
        const postData = await getCommunityPostDetail(postId);
        setTitle(postData.title);
        setContent(postData.content);
        setSelectedKeyword(postData.keyword);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      } else if (selectedImage === originalImageUrl && originalImageUrl) {
        // 기존 이미지를 그대로 사용
      ...
      await updateCommunityPost(postId, postData);
